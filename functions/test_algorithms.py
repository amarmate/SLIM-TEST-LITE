...
def test_slim(X_train, y_train, X_test, y_test,
    ...
    for it in tqdm(range(n_samples), disable=not show_progress):
        ...
        if scale:
            ...
            y_pred_descaled = torch.tensor(scaler_y.inverse_transform(y_pred_np.reshape(-1, 1)).reshape(-1))
            y_test_descaled = torch.tensor(scaler_y.inverse_transform(y_test_np.reshape(-1, 1)).reshape(-1))
